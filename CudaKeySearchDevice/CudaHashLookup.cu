...
*/
cudaError_t CudaHashLookup::setTargetConstantMemory(
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    cudaError_t err =
        cudaMemcpyToSymbol(_TARGET_HASH, h, sizeof(unsigned int) * 5,
                           i * sizeof(unsigned int) * 5);
...
/**
Returns the optimal bloom filter size in bits given the probability of
false-positives and the number of hash functions
*/
...
void CudaHashLookup::initializeBloomFilter(
    const std::vector<struct hash160> &targets, unsigned int *filter,
    unsigned int mask) {
  // Use the low 16 bits of each word in the hash as the index into the bloom
  // filter
...
void CudaHashLookup::initializeBloomFilter64(
    const std::vector<struct hash160> &targets, unsigned int *filter,
  ...
  for (unsigned int k = 0; k < targets.size(); k++) {
    ...
    idx[2] =
        ((unsigned long long)(hash[0] ^ hash[1]) << 32 | (hash[1] ^ hash[2])) &
        mask;
    idx[3] =
        ((unsigned long long)(hash[2] ^ hash[3]) << 32 | (hash[3] ^ hash[4])) &
        mask;
    idx[4] =
        ((unsigned long long)(hash[0] ^ hash[3]) << 32 | (hash[1] ^ hash[3])) &
        mask;
...
*/
cudaError_t CudaHashLookup::setTargetBloomFilter(
    const std::vector<struct hash160> &targets) {
  unsigned int bloomFilterBits =
      getOptimalBloomFilterBits(1.0e-9, targets.size());

  unsigned long long bloomFilterSizeWords = (unsigned long long)1
                                            << (bloomFilterBits - 5);
  unsigned long long bloomFilterBytes = (unsigned long long)1
                                        << (bloomFilterBits - 3);
  unsigned long long bloomFilterMask =
      (((unsigned long long)1 << bloomFilterBits) - 1);

  Logger::log(LogLevel::Info,
              "Allocating bloom filter (" +
                  util::format("%.1f", (double)bloomFilterBytes /
                                           (double)(1024 * 1024)) +
                  "MB)");
  ...
  if (err) {
    Logger::log(LogLevel::Error,
                "Device error: " + std::string(cudaGetErrorString(err)));
    delete[] filter;
    return err;
  }
  ...
  // Copy to device
  err = cudaMemcpy(_bloomFilterPtr, filter,
                   sizeof(unsigned int) * bloomFilterSizeWords,
  ...
  err = cudaMemcpyToSymbol(_BLOOM_FILTER, &_bloomFilterPtr,
                           sizeof(unsigned int *));
  if (err) {
    cudaFree(_bloomFilterPtr);
    _bloomFilterPtr = NULL;
...
/**
*Copies the target hashes to either constant memory, or the bloom filter
depending on how many targets there are
*/
cudaError_t
CudaHashLookup::setTargets(const std::vector<struct hash160> &targets) {
  cleanup();
