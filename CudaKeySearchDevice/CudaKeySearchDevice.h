...
private:
  ...
  // Telemetry and Async members
  cudaEvent_t _startEvent;
  cudaEvent_t _stopEvent;
  cudaEvent_t _memStartEvent;
  ...
  bool verifyKey(const secp256k1::uint256 &privateKey,
                 const secp256k1::ecpoint &publicKey,
                 const unsigned int hash[5], bool compressed);
