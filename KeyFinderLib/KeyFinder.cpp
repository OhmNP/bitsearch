...
void KeyFinder::setTargets(std::string targetsFile) {
  ...
  Logger::log(LogLevel::Info,
              util::formatThousands(_targets.size()) + " addresses loaded (" +
                  util::format("%.1f", (double)(sizeof(KeySearchTarget) *
                                                _targets.size()) /
                                           (double)(1024 * 1024)) +
                  "MB)");
...
void KeyFinder::stop() { _running = false; }
  ...
  while (_running) {
    ...
    if (_device->getResults(results) > 0) {
      ...
      for (unsigned int i = 0; i < results.size(); i++) {
        ...
        info.address =
            Address::fromPublicKey(results[i].publicKey, results[i].compressed);
...
secp256k1::uint256 KeyFinder::getNextKey() { return _device->getNextKey(); }