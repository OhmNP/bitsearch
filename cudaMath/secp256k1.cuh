 ...
 */
__constant__ static unsigned int _P[8] = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
...
/**
 Base point X
 */
__constant__ static unsigned int _GX[8] = {0x79BE667E, 0xF9DCBBAC, 0x55A06295,
...
/**
 Base point Y
 */
__constant__ static unsigned int _GY[8] = {0x483ADA77, 0x26A3C465, 0x5DA4FBFC,
 ...
 * Group order
 */
__constant__ static unsigned int _N[8] = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
                                          0xFFFFFFFE, 0xBAAEDCE6, 0xAF48A03B,
                                          0xBFD25E8C, 0xD0364141};

__constant__ static unsigned int _BETA[8] = {0x7AE96A2B, 0x657C0710, 0x6E64479E,
                                             0xAC3434E9, 0x9CF04975, 0x12F58995,
                                             0xC1396C28, 0x719501EE};
...
__device__ __forceinline__ static void copyBigInt(const unsigned int src[8],
                                                  unsigned int dest[8]) {
  for (int i = 0; i < 8; i++) {
    dest[i] = src[i];
  }
 ...
 */
__device__ static void readInt(const unsigned int *ara, int idx,
                               unsigned int x[8]) {
 ...
 * Subtraction mod p
 */
__device__ static void subModP(const unsigned int a[8], const unsigned int b[8],
                               unsigned int c[8]) {
  sub_cc(c[7], a[7], b[7]);
  subc_cc(c[6], a[6], b[6]);
  subc_cc(c[5], a[5], b[5]);
...
__device__ static unsigned int add(const unsigned int a[8],
                                   const unsigned int b[8], unsigned int c[8]) {
  add_cc(c[7], a[7], b[7]);
  addc_cc(c[6], a[6], b[6]);
  addc_cc(c[5], a[5], b[5]);
  addc_cc(c[4], a[4], b[4]);
...
__device__ static unsigned int sub(const unsigned int a[8],
                                   const unsigned int b[8], unsigned int c[8]) {
  sub_cc(c[7], a[7], b[7]);
  subc_cc(c[6], a[6], b[6]);
  subc_cc(c[5], a[5], b[5]);
...
__device__ static void addModP(const unsigned int a[8], const unsigned int b[8],
                               unsigned int c[8]) {
  add_cc(c[7], a[7], b[7]);
  addc_cc(c[6], a[6], b[6]);
  addc_cc(c[5], a[5], b[5]);
...
__device__ static void mulModP(const unsigned int a[8], const unsigned int b[8],
                               unsigned int c[8]) {
  ...
  // That is, take high[0 ... 5], high6, high7, multiply by 977 and add to c[0
  // ... 7]
  ...
  // Repeat the same steps, but this time we only need to handle high[6] and
  // high[7]
  high7 = high[7];
  high6 = high[6];
...
__device__ static void invModP(const unsigned int *value,
...
__device__ __forceinline__ static void beginBatchAdd(const unsigned int *px,
                                                     const unsigned int *x,
                                                     unsigned int *chain, int i,
                                                     int batchIdx,
...
beginBatchAddWithDouble(const unsigned int *px, const unsigned int *py,
...
__device__ static void completeBatchAddWithDouble(
    const unsigned int *px, const unsigned int *py, const unsigned int *xPtr,
    const unsigned int *yPtr, int i, int batchIdx, unsigned int *chain,
    unsigned int *inverse, unsigned int newX[8], unsigned int newY[8]) {
  unsigned int s[8];
  unsigned int x[8];
  unsigned int y[8];
...
completeBatchAdd(const unsigned int *px, const unsigned int *py,
                 unsigned int *xPtr, unsigned int *yPtr, int i, int batchIdx,
