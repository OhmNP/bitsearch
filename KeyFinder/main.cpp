...
#include <iostream>
#include <stdio.h>
#include <thread> // Add this include to resolve std::this_thread
...
typedef struct RunConfig {
  ...
  // New fields for range splitting and cycling
  secp256k1::uint256 rangeSize; // Size of each split range
  uint64_t
      cycleInterval; // Time interval (in milliseconds) to cycle through ranges

  // Map to store progress for each range
  std::map<std::string, secp256k1::uint256>
      rangeProgress; // Key: range identifier, Value: nextKey for the range
...
void resultCallback(KeySearchResult info) {
  if (_config.resultsFile.length() != 0) {
    Logger::log(LogLevel::Info, "Found key for address '" + info.address +
                                    "'. Written to '" + _config.resultsFile +
                                    "'");
...
void statusCallback(KeySearchStatus info) {
  ...
  std::string totalStr =
      "(" + util::formatThousands(_config.totalkeys + info.total) + " total)";
  ...
  std::string usedMemStr =
      util::format((info.deviceMemory - info.freeMemory) / (1024 * 1024));

  std::string totalMemStr = util::format(info.deviceMemory / (1024 * 1024));

  std::string targetStr =
      util::format(info.targets) + " target" + (info.targets > 1 ? "s" : "");
  ...
  printf(formatStr, devName.c_str(), usedMemStr.c_str(), totalMemStr.c_str(),
...
bool parseKeyspace(const std::string &s, secp256k1::uint256 &start,
                   secp256k1::uint256 &end, secp256k1::uint256 &rangeSize) {
  size_t pos = s.find(':');
  size_t rangePos = s.find(',');
  ...
  if (pos == std::string::npos) {
    ...
    start = left.empty() ? secp256k1::uint256(1) : secp256k1::uint256(left);

    std::string right = s.substr(pos + 1, rangePos - pos - 1);
    end = right[0] == '+' ? start + secp256k1::uint256(right.substr(1))
                          : secp256k1::uint256(right);
  }

  if (rangePos != std::string::npos) {
    rangeSize = secp256k1::uint256(s.substr(rangePos + 1));
  } else {
    rangeSize = end - start + 1; // Default to the entire range
  }
...
void usage() {
  ...
  printf("--share M/N             Divide the keyspace into N equal shares, "
         "process the Mth share\n");
  printf("--continue FILE         Save/load progress from FILE\n");
  printf(
      "--range-size SIZE       Split the keyspace into ranges of SIZE keys\n");
  printf("--cycle-interval TIME   Time interval (in milliseconds) to cycle "
         "through ranges\n");
}
...
static KeySearchDevice *getDeviceContext(DeviceManager::DeviceInfo &device,
                                         int blocks, int threads,
                                         int pointsPerThread) {
#ifdef BUILD_CUDA
  if (device.type == DeviceManager::DeviceType::CUDA) {
    return new CudaKeySearchDevice((int)device.physicalId, threads,
                                   pointsPerThread, blocks);
  }
#endif
...
static void
printDeviceList(const std::vector<DeviceManager::DeviceInfo> &devices) {
...
void writeCheckpoint(secp256k1::uint256 nextKey) {
  ...
  tmp << "rangeSize=" << _config.rangeSize.toString() << std::endl;
  tmp << "cycleInterval=" << _config.cycleInterval << std::endl;
  // Save progress for each range
  for (const auto &range : _config.rangeProgress) {
    tmp << "range=" << range.first << "," << range.second.toString()
        << std::endl;
  }
  ...
  tmp << "elapsed=" << (_config.elapsed + util::getSystemTime() - _startTime)
      << std::endl;
...
void readCheckpointFile() {
  ...
  _config.rangeSize = secp256k1::uint256(entries["rangeSize"].value);
  _config.cycleInterval = std::stoull(entries["cycleInterval"].value);

  // Load progress for each range
  for (const auto &entry : entries) {
    if (entry.first.find("range=") == 0) {
      size_t pos = entry.second.value.find(',');
      std::string rangeId = entry.second.value.substr(0, pos);
      secp256k1::uint256 progress(entry.second.value.substr(pos + 1));
      _config.rangeProgress[rangeId] = progress;
    }
  }
...
int run() {
  if (_config.device < 0 || _config.device >= _devices.size()) {
    Logger::log(LogLevel::Error,
                "device " + util::format(_config.device) + " does not exist");
    return 1;
  }
  ...
  try {
    ...
    // Get device context
    KeySearchDevice *d =
        getDeviceContext(_devices[_config.device], _config.blocks,
                         _config.threads, _config.pointsPerThread);
...
int main(int argc, char **argv) {
  ...
  for (unsigned int i = 0; i < args.size(); i++) {
    ...
    try {
      if (optArg.equals("-t", "--threads")) {
        ...
        secp256k1::uint256 rangeSize;

        parseKeyspace(optArg.arg, start, end, rangeSize);
  ...
  // Calculate where to start and end in the keyspace when the --share option is
  // used
  if (optShares) {
    Logger::log(LogLevel::Info, "Share " + util::format(shareIdx) + " of " +
                                    util::format(numShares));
    ...
    secp256k1::uint256 startPos =
        _config.nextKey + (shareSize * (shareIdx - 1));

    if (shareIdx < numShares) {
      secp256k1::uint256 endPos =
          _config.nextKey + (shareSize * (shareIdx)) - 1;
