...
public:
  ...
  uint256() { memset(this->v, 0, sizeof(this->v)); }
  ...
  uint256(const std::string &s) {
    ...
    // Ensure the value is 64 hex digits. If it is longer, take the
    // least-significant 64 hex digits. If shorter, pad with 0's.
    if (t.length() > 64) {
  ...
  bool operator<(const uint256 &x) const { return this->cmp(x) < 0; }
  ...
  uint256 operator+(const uint256 &x) const { return add(x); }

  uint256 operator+(uint32_t x) const { return add(x); }

  uint256 operator*(uint32_t x) const { return mul(x); }

  uint256 operator*(const uint256 &x) const { return mul(x); }

  uint256 operator*(uint64_t x) const { return mul(x); }

  uint256 operator-(const uint256 &x) const { return sub(x); }
  ...
  unsigned int toInt32() const { return this->v[0]; }
  ...
  bool isEven() { return (this->v[0] & 1) == 0; }
  ...
  uint64_t toUint64() { return ((uint64_t)this->v[1] << 32) | v[0]; }
};

const unsigned int _POINT_AT_INFINITY_WORDS[8] = {
...
uint256 powModP(const uint256 &base, const uint256 &exp);
ecpoint parsePublicKey(const unsigned char *data, size_t len);
...
void generateKeyPairsBulk(const ecpoint &basePoint,
