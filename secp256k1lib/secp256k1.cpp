...
static void multiply(const unsigned int *x, int xLen, const unsigned int *y,
  ...
  // Zero out the first yLen words of the z. We only need to clear the first
  // yLen words because after each iteration the most significant word is
  // overwritten anyway
  ...
  for (i = 0; i < xLen; i++) {
    ...
    for (j = 0; j < yLen; j++) {
      ...
      // Take the existing sum and add it to the product plus the high word from
      // the previous multiplication. Since we are adding to a larger datatype,
      // the compiler will take care of performing any carries resulting from
      // the addition
...
static bool isEven(const uint256 &x) { return (x.v[0] & 1) == 0; }
...
uint256 secp256k1::negModP(const uint256 &x) { return subModP(P, x); }

uint256 secp256k1::negModN(const uint256 &x) { return subModN(N, x); }
...
static void reduceModN(const unsigned int *x, unsigned int *r) {
  unsigned int barrettN[] = {0x2fc9bec0, 0x402da173, 0x50b75fc4,
                             0x45512319, 0x00000001, 0x00000000,
                             0x00000000, 0x00000000, 00000001};
...
void secp256k1::generateKeyPairsBulk(unsigned int count,
                                     const ecpoint &basePoint,
                                     std::vector<uint256> &privKeysOut,
    ...
    for (unsigned int j = 0; j < count; j++) {
      ...
      if (k.bit(i)) {
        if (isPointAtInfinity(pubKeysOut[j])) {
          ...
          // rx = (s*s - px - qx) % _p;
          uint256 rx =
              subModP(subModP(multiplyModP(s, s), pubKeysOut[j].x), table[i].x);

          // ry = (s * (px - rx) - py) % _p;
          uint256 ry = subModP(multiplyModP(s, subModP(pubKeysOut[j].x, rx)),
                               pubKeysOut[j].y);
...
/**
 * Parses a public key. Expected format is 04<64 hex digits for X><64 hex digits
 * for Y>
 */
...
// ============================================================================
// HARDENED HARNESS EXTENSIONS
// ============================================================================

uint256 secp256k1::powModP(const uint256 &base, const uint256 &exp) {
  uint256 res(1);
  uint256 b = base;
  uint256 e = exp;

  // Standard LSB-first binary exponentiation for 256 bits
  for (int i = 0; i < 256; i++) {
    if ((e.v[0] & 1)) { // Check LSB
      res = multiplyModP(res, b);
    }
    b = multiplyModP(b, b);

    // Shift e right by 1 using internal divBy2
    e = divBy2(e);
  }
  return res;
}

ecpoint secp256k1::parsePublicKey(const unsigned char *data, size_t len) {
  if (len != 33 && len != 65) {
    throw std::string("Invalid public key length");
  }
  ...
  if (header == 0x04) {
    if (len != 65)
      throw std::string("Invalid uncompressed length");

    // Reconstruct manually from Big Endian bytes
    unsigned int x_words[8], y_words[8];
    const unsigned char *x_ptr = data + 1;
    const unsigned char *y_ptr = data + 33;

    for (int i = 0; i < 8; i++) {
      x_words[i] = ((unsigned int)x_ptr[i * 4] << 24) |
                   ((unsigned int)x_ptr[i * 4 + 1] << 16) |
                   ((unsigned int)x_ptr[i * 4 + 2] << 8) |
                   (unsigned int)x_ptr[i * 4 + 3];
      y_words[i] = ((unsigned int)y_ptr[i * 4] << 24) |
                   ((unsigned int)y_ptr[i * 4 + 1] << 16) |
                   ((unsigned int)y_ptr[i * 4 + 2] << 8) |
                   (unsigned int)y_ptr[i * 4 + 3];
    }
    // uint256 constructor with endian flag exists: uint256(x, BigEndian)
    uint256 x(x_words, uint256::BigEndian);
    uint256 y(y_words, uint256::BigEndian);
    ecpoint p(x, y);
    if (!pointExists(p))
      throw std::string("Point not on curve");
    return p;
  }
  ...
  if (header == 0x02 || header == 0x03) {
    if (len != 33)
      throw std::string("Invalid compressed length");
    ...
    uint256 x3 = multiplyModP(x2, x);
    uint256 rhs = addModP(x3, uint256(7));

    // Exponent = (P+1)/4
    uint256 exp = P.add(1).div(4);

    uint256 y = powModP(rhs, exp);

    // Verify square: y^2 == rhs
    uint256 y2 = multiplyModP(y, y);
    if (!(y2 == rhs)) {
      throw std::string("Point not on curve (X^3+7 not a square)");
    }

    // Check Parity
    // 0x02 -> Even (0), 0x03 -> Odd (1)
    bool expected_odd = (header == 0x03);
    bool found_odd = !y.isEven(); // isEven() checks bit 0 of v[0]

    if (expected_odd != found_odd) {
      y = negModP(y);
    }
  ...
  throw std::string("Invalid public key header");
}
